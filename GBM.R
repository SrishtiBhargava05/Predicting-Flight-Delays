...
summary(Delayed.Boost)
predict1 <- predict.gbm(Delayed.Boost, newdata = test, n.trees = 200)
...
Model <- c(1,2,3,4,5)
Predictors <- c("Month, DayofMonth, DayOfWeek, UniqueCarrier, Distance, DepHour",
                "Month, DayOfWeek, UniqueCarrier, Origin, Dest, DepHour",
                "Month, DayOfWeek, UniqueCarrier, Origin, Dest, DepHour",
                "UniqueCarrier, Distance, DepHour",
                "UniqueCarrier, Distance, DepHour")
CrossValidation <- c("2-fold","3-fold", "2-fold","2-fold","2-fold")
NumberOfTrees <- c(200,200,150,200,400)
Depth <- c(5,6,3,6,8)

ModelSummary <- data.frame(Model,Predictors,CrossValidation,NumberOfTrees,Depth)
ModelSummary
